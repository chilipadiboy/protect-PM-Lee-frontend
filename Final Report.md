...
`External Partners` will have a page for them to upload their database file as a .csv file. This page is intended to let external parties to include their pantients in the Hospital's main database. Doing so will let them use the other functionalities such as uploading records and files into the system.

The CSV file should be in a fixed format with the columns in the following order without headers:
...
1. Phone
1. Password
1. Private Key (Generated by admin if needed)
1. Public Key (Generated by admin if needed)
1. Role (Admin role not allowed)
